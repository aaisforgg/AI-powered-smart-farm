...
def dibujar_hud(pantalla, state, agente, fuentes):
    ...
    _label(pantalla, fuentes, "AI SMART FARM",       hx + pad, hy + 8, C["accent"],  "md")
    ...
    _section_title(pantalla, fuentes, "ESTACIÓN", sx + pad, sy + pad - 5, sw - pad * 2)
    _label(pantalla, fuentes, season, sx + pad, sy + 26, s_color, "sm")
    ...
    _section_title(pantalla, fuentes, "AGENTE", ax + pad, ay + pad - 5, aw - pad * 2)
    ...
    _section_title(pantalla, fuentes, f"GENÉTICA  —  Gen {gen_num}", gx + pad, gy + pad - 5, gw - pad * 2)
    ...
    for i, (lbl, val, pct) in enumerate(gene_stats):
        ...
        _label(pantalla, fuentes, lbl, bx,      by - 5, C["txt_dim"], "xs")
        _label(pantalla, fuentes, val, bx + 40, by - 5, C["txt_hi"],  "xs")
    ...
    _section_title(pantalla, fuentes, evo_title, vx + pad, vy + pad - 5, vw - pad * 2)
    ...
    _section_title(pantalla, fuentes, f"CULTIVOS  ({total} total)", cx + pad, cy_s + pad - 5, cw - pad * 2)
    for i, (fase, label) in enumerate(CROP_PHASE_LABELS.items()):
        bx  = cx + pad
        by  = crop_row0_y + i * crop_row_h - 10
        cnt = fase_counts.get(fase, 0)
        pygame.draw.circle(pantalla, CROP_PHASE_COLORS[fase], (bx + 4, by + 6), 4)
        _label(pantalla, fuentes, label, bx + 14, by - 5, C["txt_mid"], "xs")
        cnt_w = fuentes["xs"].size(str(cnt))[0]
        _label(pantalla, fuentes, str(cnt), cx + cw - pad - cnt_w - 10, by - 6, C["txt_hi"], "xs")
        if total > 0:
